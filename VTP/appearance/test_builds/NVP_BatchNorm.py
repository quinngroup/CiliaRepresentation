...
class VAE(nn.Module):
    def __init__(self, input_length, lsdim, device, logvar_bound):
        ...
        self.res1_x = nn.ModuleList([Residual(pixelwise_channel=32, conv_channel=16,stride=2 if k==0 else 1) for k in range(4)])
        self.pw_conv1 = nn.Conv2d(32, 64, 1)
        self.batch2 = nn.BatchNorm2d(64)
        self.res2_x = nn.ModuleList([Residual(pixelwise_channel=64, conv_channel=32,stride=2 if k==0 else 1) for k in range(10)])
        self.pw_conv2 = nn.Conv2d(64, 256, 1)
        self.batch3 = nn.BatchNorm2d(256)
        self.res3_x = nn.ModuleList([Residual(pixelwise_channel=256, conv_channel=128,stride=2 if k==0 else 1) for k in range(6)])
