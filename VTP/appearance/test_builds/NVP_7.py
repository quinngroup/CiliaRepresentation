...
class VAE(nn.Module):
    def __init__(self, input_length, lsdim, device, logvar_bound):
        ...
        #Encoding Layers
        self.pw_conv0 = nn.Conv2d(1, 32, 1)
        ...
        self.sbd=spatial_broadcast_decoder(input_length=self.input_length,device=self.device,lsdim=self.lsdim,channels=[64,64,64,64,32])
        self.res1d_x = nn.ModuleList([Residual(pixelwise_channel=32, conv_channel=16) for k in range(3)])
        self.pwd_conv1 = nn.Conv2d(32, 64, 1)
        self.res2d_x = nn.ModuleList([Residual(pixelwise_channel=64, conv_channel=32) for k in range(12)])
        self.pwd_conv2 = nn.Conv2d(64, 256, 1)
        self.res3d_x = nn.ModuleList([Residual(pixelwise_channel=256, conv_channel=128) for k in range(5)])
        self.pwd_conv3 = nn.Conv2d(256, 1, 1)
    ...
    def decode(self, z):
        x = self.sbd(z)
        for r in self.res1d_x:
            x = r(x)

        x = F.leaky_relu(self.pwd_conv1(x))

        for r in self.res2d_x:
            x = r(x)

        x = F.leaky_relu(self.pwd_conv2(x))

        for r in self.res3d_x:
            x = r(x)

        x = F.leaky_relu(self.pwd_conv3(x))
